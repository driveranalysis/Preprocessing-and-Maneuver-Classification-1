...
actnames = getActivityNames(); %#ok<NASGU>
...
save(fullfile(destDataPath,'BufferedAccelerations.mat'),...
    'atx','aty','atz','subid','actid','fs','t','-v7.3')
fprintf('Done.\n')
...
% Add [<many> x 3] acceleration array to use for streaming simulation
at = vertcat(subjects(:).totalacc); %#ok<NASGU>
actid = vertcat(subjects(:).actid); %#ok<NASGU>

fprintf('Saving data for streaming simulation...')
save(fullfile(destDataPath,'ContinuousAccelerations.mat'),...
    'at','actid','fs','-v7.3')
fprintf('Done.\n')
